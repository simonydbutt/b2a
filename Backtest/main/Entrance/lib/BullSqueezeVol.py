...
class BullSqueezeVol:
    ...
    def __init__(self, df, params):
        ...
        self.condList = (
            params["condList"]
            if "condList" in params.keys()
            else ("isWhite", "maBelow", "volCond")
        )
        self.df = Attr(
            Attr(
                Attr(
    ...
    def volCond(self):
        ...
        for i in range(self.numBSPeriods, len(self.df)):
            row = self.df.iloc[i]
            if (
                self.df.iloc[i - self.numBSPeriods - 1]["takerBaseAssetVol"]
                > row["bolVolUp"]
                and row["takerBaseAssetVol"] < (row["meanVol"] + row["bolVolUp"]) / 2
            ):
                volList[i] = True
        return volList

...
class CandlestickChart:
    ...
    def plot(self, df, granularity, MA=False, Bollinger=False, Vol=False, volName=""):
        ...
        if Vol:
            ...
            p = figure(
                ...
                y_range=(
                    min(df.low) - priceRange / coef,
                    max(df.high) + 0.1 * priceRange,
                ),
            )
        else:
        ...
        if Vol:
            ...
            p.extra_y_ranges = {
                "Vol": Range1d(start=0, end=float(max(df[volName]) * 4))
            }
            p.rect(
                df["date"],
                df[volName] / 2,
    ...
    def plotEx(self, df, field, num=1, range=20, volName="takerBaseAssetVol"):
        ...
        while j < num and i < len(targetCol):
            if targetCol.iloc[i]:
                self.plot(
                    df.iloc[
                        int(max(0, i - (range / 2))) : int(
                            min(i + (range / 2), len(df))
                        )
                    ],
                    granularity=gran,
                    Vol=True,
    ...
    def plotStrat(self, df, timeStart, timeEnd, vol=False):
        ...
        if "ma" in df_.keys() and "bollinger" in df_.keys():
            self.plot(
                df=df_, granularity=gran, MA=["ma"], Bollinger="bollinger", Vol=vol
            )
        else:

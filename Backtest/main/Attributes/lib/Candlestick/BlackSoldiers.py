class BlackSoldiers:
    def __init__(self, df, params):
        self.df = df
        self.attrName = (
            params["attrName"] if "attrName" in params.keys() else "blackSoldiers"
        )
    ...
    def run(self):
        ...
        for i in range(self.numPeriods + self.delay, len(self.df)):
            isDecrease = False not in [
                True
                if self.df["close"].iloc[j] < self.df["close"].iloc[j - 1]
                else False
                for j in range(i - self.numPeriods - self.delay, i - self.delay)
            ]
            isBlack = False not in [
                True if self.df["close"].iloc[j] < self.df["open"].iloc[j] else False
                for j in range(i - self.numPeriods - self.delay, i - self.delay)
            ]

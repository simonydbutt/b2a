...
class OutsideUp:
    def __init__(self, df, params):
        self.df = df
        self.attrName = (
            params["attrName"] if "attrName" in params.keys() else "outsideUp"
        )
    ...
    def run(self):
        ...
        for i_ in range(1 + self.delay, len(self.df)):
            ...
            if initRange > 0:
                ...
                if (
                    rowRange > 0
                    and row["open"] < initRow["close"]
                    and row["close"] > initRow["open"]
                    and rowRange > self.coef * initRange
                ):
                    oUList[i_] = True
            else:
        ...
        if not self.bullConfirm:
            ...
            return [(self.attrName, oUList), ("bullConf", initMax)]

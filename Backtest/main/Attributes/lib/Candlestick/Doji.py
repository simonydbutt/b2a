class Doji:
    ...
    def longLeggedDoji(self, row):
        ...
        if self.dir.lower() == "up":
            return (
                row["high"] - row["close"] >= self.uCoef / 2.5 * spread
                and row["close"] - row["low"] >= self.dCoef / 2.5 * spread
                and row["close"] - row["open"] < self.coef * spread
            )
        elif self.dir.lower() == "down":
            return (
                row["high"] - row["open"]
                >= self.uCoef / 2.5 * (row["open"] - row["close"])
                and row["close"] - row["low"] >= self.dCoef / 2.5 * spread
                and row["open"] - row["close"] < self.coef * spread
            )
        else:
            return (
                row["high"] - max(row["open"], row["close"]) >= self.uCoef / 3 * spread
                and min(row["open"], row["close"]) - row["low"]
                >= self.dCoef / 3 * spread
                and abs(row["close"] - row["open"]) < self.coef * spread
            )
    ...
    def dragonDoji(self, row):
        ...
        if self.dir.lower() == "up":
            return (
                row["high"] - row["close"] <= self.uCoef * spread
                and row["close"] - row["low"] >= self.dCoef * spread
                and row["close"] - row["open"] < self.coef * spread
            )
        elif self.dir.lower() == "down":
            return (
                row["high"] - row["open"] <= self.uCoef * spread
                and row["close"] - row["low"] >= self.dCoef * spread
                and row["open"] - row["close"] < self.coef * spread
            )
        else:
            return (
                row["high"] - max(row["open"], row["close"]) <= self.uCoef * spread
                and min(row["open"], row["close"]) - row["low"] >= self.dCoef * spread
                and abs(row["close"] - row["open"]) < self.coef * spread
            )

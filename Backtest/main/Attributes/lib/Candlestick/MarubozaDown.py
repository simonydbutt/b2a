class MarubozaDown:
    def __init__(self, df, params):
        ...
        self.attrName = (
            params["attrName"] if "attrName" in params.keys() else "marubozaDown"
        )
    ...
    def lsMara(self, row):
        return (
            row["high"] == row["open"]
            and self.coef * (row["close"] - row["low"]) < row["open"] - row["close"]
        )

    def usMara(self, row):
        return (
            row["low"] == row["close"]
            and self.coef * (row["high"] - row["open"]) < row["open"] - row["close"]
        )

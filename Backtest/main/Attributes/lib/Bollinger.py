...
class Bollinger:
    ...
    def __init__(self, df, params):
        ...
        self.attrName = (
            params["attrName"] if "attrName" in params.keys() else "bollinger"
        )
        self.numPeriods = params["numPeriods"] if "numPeriods" in params.keys() else 24
        self.MAField = (
            params["maField"]
            if "maField" in params.keys()
            else "ma%s" % self.numPeriods
        )
    ...
    def run(self):
        ...
        for i in range(self.numPeriods, len(self.df)):
            ...
            bollingerUp.append(
                round(float(self.df[self.MAField].iloc[i] + self.numStd * std), 6)
            )
            bollingerDown.append(
                round(float(self.df[self.MAField].iloc[i] - self.numStd * std), 6)
            )
        return [

...
def before():
    ...
    updatePosData = pd.DataFrame(data=[[10], [9], [8], [11], [7]], columns=["close"])
    ...
    return (
        PR,
        P,
        {"main": posDataMain, "init": posDataInit, "periods": posDataPeriods},
        updatePosData,
    )
...
def test_profitRun():
    ...
    # update & initial
    assert not PR.run(
        positionData=posDict["init"], testPrice=10, testData=updatePosData, Pull=P
    )[0]
    # update & norm
    assert not PR.run(
        positionData=posDict["main"], testPrice=12, testData=updatePosData, Pull=P
    )[0]
    # in normal range

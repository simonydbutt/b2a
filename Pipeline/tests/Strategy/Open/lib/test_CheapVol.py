...
params = {
    "assetSelection": {"exchangeList": ["Binance"]},
    "enter": {
        "name": "CheapVol",
        "granularity": 7200,
        "periodsVolLong": 5,
...
initSingleData = pd.DataFrame(
    [[10, 10], [10, 10], [10, 10], [10, 10], [10, 10]], columns=["close", "volume"]
)
...
def test_initSingle():
    ...
    logging.debug(
        "Asserting %s == %s" % (col.find_one({"asset": "LTC"}, {"_id": 0}), initData)
    )
...
def test_initiateCollection():
    before()
    fL = CheapVol(
        stratName="testCheapVol",
        isTest=True,
        assetList=[("LTC", "Binance"), ("XMR", "Binance"), ("NotAnAsset", "Binance")],
    ).initiateCollection()
    logging.debug("Asserting %s == %s" % (fL, ["NotAnAsset"]))
    assert fL == ["NotAnAsset"]
    logging.debug(
        "Asserting %s == %s"
        % ([val["asset"] for val in list(col.find())], ["LTC", "XMR"])
    )
...
def test_before():
    ...
    cV = CheapVol(
        stratName="testCheapVol",
        isTest=True,
        assetList=[("LTC", "Binance"), ("XMR", "Binance")],
    )
    ...
    assert {
        val["asset"]: {"price": val["price"][-1], "vol": val["vol"][-1]}
        for val in list(col.find())
    } == newData

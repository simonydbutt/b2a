...
class UpdatePosition:
    ...
    def update(self, positionDict, currentPrice, dir="buy"):
        ...
        positionDict["paperSize"] = (
            round(
                float(positionDict["positionSize"])
                * float(currentPrice)
                / float(positionDict["openPrice"]),
                8,
            )
            if dir == "buy"
            else round(
        ...
        logging.debug(
            "New vals: periods: %s, paperSize: %s, currentPrice: %s"
            % (
                positionDict["periods"],
                positionDict["paperSize"],
                positionDict["currentPrice"],
            )
        )
        self.col.find_one_and_replace(
            {"assetName": positionDict["assetName"]}, positionDict
        )

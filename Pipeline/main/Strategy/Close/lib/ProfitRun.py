...
class ProfitRun:
    ...
    def updatePosition(self, positionData, Pull, testData=None):
        ...
        data = (
            Pull.candles(
                asset=positionData["assetName"],
                limit=self.configParams["maPeriods"],
                lastReal=True,
                interval=self.configParams["granularity"],
                columns=["close"],
        ...
        positionData["hitPrice"] = np.round(
            positionData["currentPrice"] + self.configParams["stdDict"]["up"] * std, 8
        )
        positionData["sellPrice"] = np.round(
            positionData["currentPrice"] - self.configParams["stdDict"]["down"] * std, 8
        )
        del positionData["_id"]
        self.col.find_one_and_replace(
            {"assetName": positionData["assetName"]}, positionData
        )
    ...
    def run(self, positionData, Pull, testPrice=None, testData=None):
        ...
        price = (
            Pull.assetPrice(
                exchange=positionData["exchange"], asset=positionData["assetName"]
            )
            if not self.isTest
            else testPrice
        )
        if "hitPrice" not in positionData.keys():
            ...
            if positionData["periods"] > self.configParams["closePeriods"]:
                logging.debug(
                    "Ending ProfitRun.run -> True (timeouts), price: %s" % price
                )
                return True, price
                ...
                logging.debug(
                    "Ending ProfitRun.run -> True (price dip), price: %s" % price
                )
                return True, price
            else:
                logging.debug("Ending ProfitRun.run -> False, price: %s" % price)

...
class SetupUtil:
    ...
    def createConfigs(self, configDict, baseStrat, stratName):
        response = '0'
        filePath = '%s/Pipeline/DB/Configs/%s/%s.yml' % (Settings.BASE_PATH, baseStrat, stratName)
        ...
        if response == '1' or response == '2' or response == '0':
            ...
            with open('%s/Pipeline/DB/Configs/%s/%s' % (Settings.BASE_PATH, baseStrat, fileName), 'w') as file:
                yaml.dump(configDict, file)
            TinyDB('%s/Pipeline/DB/CurrentPositions/%s/%s.ujson' %
                   (Settings.BASE_PATH, baseStrat, configDict['stratID']))
            TinyDB('%s/Pipeline/DB/PerformanceLogs/%s/StratLogs/%s.ujson' %
                   (Settings.BASE_PATH, baseStrat, configDict['stratID']))
            print(log)
        else:

...
class EmailUtil:
    ...
    def _sendEmail(self, subject, content, imgPath=None, html=False):
        ...
        server.starttls()
        server.login(user=Settings.EMAIL["USER"], password=Settings.EMAIL["PASSWORD"])
        msg = MIMEMultipart()
        msg["From"] = Settings.EMAIL["USER"]
        msg["To"] = Settings.EMAIL["SEND_USER"]
        msg["Subject"] = subject
        msg.attach(
            MIMEText(content, _subtype="plain")
            if not html
            else MIMEText(content, _subtype="html")
        )
        if imgPath:
            msg.attach(
                MIMEImage(open(imgPath, "rb").read(), name=os.path.basename(imgPath))
            )
        server.send_message(
            msg=msg,
            from_addr=Settings.EMAIL["USER"],
            to_addrs=Settings.EMAIL["SEND_USER"],
        )
    ...
    def errorExit(self, file, funct, message):
        self._sendEmail(
            ...
            content="File:\t\t\t  %s\n"
            "Function:\t     %s\n"
            "Error message:\t%s" % (file, funct, message),
        )
    ...
    def statsMessage(self):
        ...
        msg = (
            '<html lang = "en"><body><h2> B2A Performance Stats: %s</h2>'
            "<h3>Total Stats</h3>%s<br><h3><h3>Current Positions</h3>%s"
            % (
                self.strat,
                currentDF.set_index(keys="Total Stats", drop=True).to_html(),
                StatsUpdate()
                .getCurrentStats(stratName=self.strat)
                .set_index(keys="assetName", drop=True)
                .to_html(),
            )
        )
        if self.isTick:
            ...
            msg += (
                "<h3>Market Tick Data</h3>%s"
                % tickDF.set_index("Period", drop=True).to_html()
            )
        msg += "</body></html>"
        self._sendEmail(
            subject="b2a Performance Stats: %s"

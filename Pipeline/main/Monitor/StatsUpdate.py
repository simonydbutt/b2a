...
class StatsUpdate:
    ...
    def indivStats(self, stratName):
        ...
        with open(
            "%s/Pipeline/resources/%s/capital.yml" % (Settings.BASE_PATH, stratName)
        ) as capFile:
        ...
        stats["paperAvgPnL"] = (
            round(
                stats["paperPnL"] / (stats["numberOpen"] + stats["numberTransactions"]),
                4,
            )
            if stats["numberOpen"] + stats["numberTransactions"] != 0
            else 0
        )
    ...
    def compStats(self, isTest=None):
        ...
        stratList = (
            [
                val
                for val in os.listdir("%s/Pipeline/resources" % Settings.BASE_PATH)
                if val != "__init__.py"
            ]
            if not isTest
            else isTest
        ...
        totalStats["paperPnL"] = round(
            100 * (totalStats["paperCurrent"] / totalStats["initialCapital"] - 1), 4
        )
        totalStats["percentAllocated"] = 100 * round(
            1 - totalStats["liquidCurrent"] / totalStats["paperCurrent"], 2
        )
    ...
    def getCurrentStats(self, stratName):
        ...
        df = pd.DataFrame(
            list(
                self.client[stratName].currentPositions.find(
                    {},
                    {
                        "_id": 0,
                        "assetName": 1,
